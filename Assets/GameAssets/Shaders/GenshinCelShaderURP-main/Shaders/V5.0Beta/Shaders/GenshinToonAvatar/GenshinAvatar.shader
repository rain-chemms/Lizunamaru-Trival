Shader "GenshinCelShaderURP/V5.0Beta"
{
    Properties
    {
        ...
        //是否只显示边缘相关的信息
        [Toggle(_ONLY_OUTLINE_RIM_ON)] _OnlyOutlineAndRim("Only Render Outline & Rim (Hide Body)", Float) = 0
        ...
        // 修改后:改为Alpha混合
        //[Enum(UnityEngine.Rendering.BlendMode)] _SrcBlendModeColor("Src Blend Mode (Default SrcAlpha)", Float) = 5

        // 原来的混合
        [Enum(UnityEngine.Rendering.BlendMode)] _DstBlendModeColor("Core Pass dst blend mode color (Default Zero)", Float) = 0
        // 修改后: 修改为OneMinusSrcAlpha
        //[Enum(UnityEngine.Rendering.BlendMode)] _DstBlendModeColor("Dst Blend Mode (Default OneMinusSrcAlpha)", Float) = 10
        ...
        // 修改后:关闭ZWrite
        //[Enum(Off, 0, On, 1)] _ZWrite("ZWrite (Default Off)", Int) = 0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //声明宏:便于Special Effect生效
            #pragma shader_feature_local _ONLY_OUTLINE_RIM_ON
            
            /*
            // 新添加的Shader字段
            #pragma shader_feature_local _OUTLINE_ONLY_ON
            // [OutlineOnly] 改用包装函数
            #pragma fragment GenshinStyleFragmentWrapper 

            // [OutlineOnly] 包装片段着色器：开启时直接 discard，不渲染本体
            #if defined(_OUTLINE_ONLY_ON)
                clip(-1); // 丢弃所有片段，不渲染本体
